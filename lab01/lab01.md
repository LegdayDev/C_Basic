...
### 10진법 기본
- 10진법이란 인간이 사용하는 일반적인 진법체계이다.
- 한 자리 숫자는 10가지 경우가 존재(0~9)
- 자릿수가 올라갈 때마다 10의 n제곱을 곱한다.
- 10진수 512 를 풀어보면 다음과 같다.
  - `(5*10의2승)+(1*10의1승)+(2*10의0승) = 512`
- IT쪽에선는 10진수↔2진수↔16진수 를 알고 있어야 한다.

### 2진수, 16진수 변환
- 16진수의 가장 큰 특징은 10진수 기준 10부터 15까지는 A에서 F로 표기한다.
  
<img src="/etc/img06.png" width="386" height="386">
<img src="/etc/img07.png" width="388" height="386">

- 4bit로 이루어진 2진수는 16진수 한 자리를 의미한다.
- 특히 _IT쪽에서는 2진수표현보다는 16진수를 많이 쓴다._
- 하지만 16진수로만 이루어져 있으면 10진수와 헷갈릴 수 있다. 그래서 `0x` 라는 prefix가 붙게 된다.
  - `0xF` -> 2진수 1111이다.
  - `0x3F` -> **16진수 3**은 `0011` 이고, **16진수 F**는 `1111` 이다. 즉 2진수 `00111111` 이므로 **10진수 63**이다.
  - `0x3DFA` -> **16진수 3**은 `0011`, **16진수 D**는 `1101`, **16진수 F**는 `1111`, **16진수 A**는 `1010` 이므로 2진수 `0011110111111010` 이다.

### 16진수 표기가 사용되는 예
> 16진수 표기가 사용되는 예시는 가장 크게 **64bit 컴퓨터에서 메모리 화면**이다.<br>
 
<img src="/etc/img08.png" width="734" height="490">
- 위 그림을 보면 총 16자리의 16진수로 되어있는데 16진수 각 한자리를 4bit 를 차지하기 때문에 총 64bit 가 표현된다.
